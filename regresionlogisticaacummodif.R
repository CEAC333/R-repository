...
x<-gh$mids
y<-(sum(gh$counts)-cumsum(gh$counts))/(sum(gh$counts+bh$counts)-(cumsum(gh$counts)+cumsum(bh$counts)))
#esta forma de acumular pporque cuando hacemos el corte en la pureza en este caso 
#nos interesa saber los numeros mayores al corte(drecha de grafic) por eso hicimos esta modif.
plot(x=x,y=y,main="pureza")
lines(x,y)

x<-gh$mids
y<-(sum(gh$counts+bh$counts)-cumsum(gh$counts+bh$counts))/sum(gh$counts+bh$counts)
